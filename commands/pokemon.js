...
/*******************
 * Pokemon Commands
 * Lots of this code, most of it actually is lovingly borrowed from https://github.com/Zarel/Pokemon-Showdown
 *******************/
...
Tools.data = Tools.Dex.includeData().data; //LOL
 
function dataSearch(args) {
    let result = runDexsearch(args, 'data', true, args);
    if (result.reply != "``No Pokemon found.``") {
        return result.reply
    }
    result = runMovesearch(args, 'data', true, args);
    if (result.reply != "No moves found.") {
        return result.reply
    }
    result = runItemsearch(args, 'data', true, args);
    if (result.reply != "No items found. Try a more general search.") {
        return result.reply
    }
}
...
function runDexsearch(target, cmd, canAll, message) {
	...
	for (let i = 0; i < andGroups.length; i++) {
		...
		for (let j = 0; j < parameters.length; j++) {
			...
			let targetAbility = Tools.Dex.getAbility(target);
			...
			if (target === 'priority') {
				...
				for (let move in Tools.data.Movedex) {
					let moveData = Tools.Dex.getMove(move);
			...
			let targetMove = Tools.Dex.getMove(target);
	...
	for (let pokemon in Tools.data.Pokedex) {
		let template = Tools.Dex.getTemplate(pokemon);
	...
	for (let group = 0; group < searches.length; group++) {
		...
		for (let mon in dex) {
			...
			for (let type in alts.resists) {
				let effectiveness = 0;
				let notImmune = Tools.Dex.getImmunity(type, dex[mon]);
				if (notImmune) effectiveness = Tools.Dex.getEffectiveness(type, dex[mon]);
			...
			for (let move in alts.moves) {
				let movedata = Tools.data.Learnsets[mon];
				if (movedata && movedata.learnset[move]) {
					matched = true;
				}
...
function runMovesearch(target, cmd, canAll, message) {
	...
	for (let i = 0; i < targets.length; i++) {
		...
		if (typeIndex >= 0) {
			target = target.charAt(0).toUpperCase() + target.substring(1, typeIndex);
			if (!(target in Tools.data.TypeChart)) return {reply: "Type '" + target + "' not found."};
		...
		let template = Tools.Dex.getTemplate(target);
		if (template.exists) {
			if (Object.keys(lsetData).length) return {reply: "A search can only include one Pok\u00e9mon learnset."};
			if (!template.learnset) template = Tools.Dex.getTemplate(template.baseSpecies);
			lsetData = Object.assign({}, template.learnset);
			targetMon = template.name;
			while (template.prevo) {
				template = Tools.Dex.getTemplate(template.prevo);
	...
	if (targetMon) {
		for (let move in lsetData) {
			dex[move] = Tools.Dex.getMove(move);
		}
	} else {
		for (let move in Tools.data.Movedex) {
			dex[move] = Tools.Dex.getMove(move);
	...
	if (targetMon) {
		...
		resultsStr += (message === "" ? message : "<span style=\"color:#999999;\">" + message + ":</span><br />");
	}
	if (results.length > 0) {
		if (showAll || results.length <= RESULTS_MAX_LENGTH + 5) {
			results.sort();
...
function runItemsearch(target, cmd, canAll, message) {
	...
	if (searchedWords.includes('fling')) {
		...
		for (let l = 0; l < foundItems.length; l++) {
			let item = Tools.Dex.getItem(foundItems[l]);
			let matched = 0;
			let descWords = item.desc;
			if (/[1-9\.]+x/.test(descWords)) descWords += ' increases';
			if (item.isBerry) descWords += ' berry';
	...
	let resultsStr = (message === "" ? message : "<span style=\"color:#999999;\">" + message + ":</span><br />");
...
exports.commands = {
    ...
    ds: 'dexsearch',
    dexsearch: function(args) {
        if (!this.can('broadcast')) return this.deny();
